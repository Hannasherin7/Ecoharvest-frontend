...
export const ProductList = () => {
  ...
  const toggleDescription = (productId, field) => {
  ...
  const checkProductAvailability = async (productId, postalCode) => {
    try {
      const response = await axios.post(
        "http://localhost:7000/check-availability",
        {
          productId,
          postalCode,
        },
  ...
  const placeOrder = async () => {
    if (
      ...
      const availabilityResponse = await checkProductAvailability(
        selectedProduct._id,
        orderData.pincode
      );
  ...
  const filteredProducts = products.filter((product) => {
    const matchesSearch = product.pname
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesCategory = selectedCategory
      ? product.category === selectedCategory
      : true;
    const isOrganicProduct = product.productype === "Organic Product";
    return (
      matchesSearch &&
      matchesCategory &&
  ...
  const renderStars = (rating) => {
    return Array(5)
      ...
      .map((_, i) =>
        i < rating ? (
          <FaStar key={i} style={{ color: "#FFA000" }} />
        ) : (
          <FaRegStar key={i} style={{ color: "#FFA000" }} />
        )
      );
  };
  ...
  const shareProduct = (product) => {
    if (navigator.share) {
      navigator
        .share({
          title: product.pname,
          text: `Check out this ${product.category} product on EcoHarvest`,
  ...
  const renderExpandableText = (text, productId, field, maxLength = 100) => {
    if (!text) return null;
    
    const isExpanded = expandedDescriptions[productId]?.[field];
    ...
    return (
      <div style={{ marginBottom: "15px" }}>
        ...
        {shouldTruncate && (
          <button 
            onClick={() => toggleDescription(productId, field)}
            style={{
              background: "none",
              border: "none",
              color: "#4CAF50",
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: "400px",
          background:
            "linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('https://t3.ftcdn.net/jpg/02/48/92/96/360_F_248929619_JkVBYroM1rSrshWJemrcjriggudHMUhV.jpg')",
      ...
      >
        ...
        >
          Discover fresh, organic products for a healthier lifestyle. Perfect
          for home cooking and professional chefs.
        </p>
                ...
                {categories.map((category) => (
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      backgroundColor:
                        selectedCategory === category ? "#4a934a" : "#f0f0f0",
                      ...
                      ":hover": {
                        backgroundColor:
                          selectedCategory === category ? "#3d8b40" : "#e0e0e0",
                      },
                    }}
                  >
                    {category}
                    ...
                    >
                      ...
                      <div className="card-body" style={{ padding: "20px" }}>
                        ...
                        >
                          seller:
                          <FaUser
                            style={{ color: "#777", marginRight: "8px" }}
                          />
                          <span style={{ color: "#555" }}>
                            {product.userId?.name || "Anonymous Seller"}
                          </span>
                        </div>

                        {/* Description with Read More */}
                        {renderExpandableText(product.discription, product._id, "description")}

                        {/* Details with Read More */}
                        {renderExpandableText(product.details, product._id, "details")}

                        {/* Special Offers with Read More */}
                        {renderExpandableText(product.specialOffers, product._id, "specialOffers")}
